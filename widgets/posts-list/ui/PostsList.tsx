...
export const PostsList = ({ className }: PostsListProps) => {
  ...
  return (
    <div className={`${styles.postsList} ${className || ''}`}>
              ...
              <div className={styles.postInfo}>
                <div className={styles.userInfo}>
                  <span className={styles.username}>
                    {post.userName || post.owner?.firstName || 'Автор'}
                  </span>
                </div>
                ...
                <div className={styles.postMeta}>
                  <span className={styles.likesCount}>
                    {formatLikes(post.likesCount || 0)} likes
                  </span>
        ...
        {isLoading && (
          <div className={styles.loadingSkeletons}>
            {Array.from({ length: 10 }).map((_, index) => (
              <Card key={`skeleton-${index}`} className={styles.postCard}>
                <div className={styles.postContent}>
                  <div className={styles.postImageContainer}>
        ...
        {isLoadingMore && (
          <div className={styles.loadingMore}>
            {Array.from({ length: 5 }).map((_, index) => (
              <Card key={`more-skeleton-${index}`} className={styles.postCard}>
                <div className={styles.postContent}>
                  <div className={styles.postImageContainer}>
                    <Skeleton className={styles.imageSkeleton} />
                  </div>
      ...
      <div ref={loaderRef} className={styles.intersectionObserverTrigger} />
